...
app.get("/dbcheck", async (_req, res) => {
  try {
    if (!supabase) return res.status(500).json({ db: "fail", detalle: "vars faltantes" });
    const { error } = await supabase.from("comentarios").select("id", { head: true, count: "exact" }).limit(1);
    if (error) throw error;
    res.json({ db: "ok" });
  } catch (e) {
    console.error("dbcheck error:", e.message || e);
    res.status(500).json({ db: "fail", detalle: String(e.message || e) });
  }
});
